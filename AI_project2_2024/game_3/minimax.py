...
def minimax(cur_player_id, map_state, sheep_state, depth, alpha, beta, max_player_id):
    ...
    if depth == 0:
        return evaluation(max_player_id, map_state, sheep_state)

    next_player = player_list[cur_player_id % 4]

    if next_player == max_player_id:
        max_score = -1000
        ...
        if len(moves)==0:
            max_score=minimax(next_player, map_state, sheep_state, depth - 1, alpha, beta, max_player_id)
        ...
        for move in moves:
            ...
            if beta <= alpha:
                break
        if len(moves)==0:
            min_score=minimax(next_player, map_state, sheep_state, depth - 1, alpha, beta, max_player_id)
        return min_score
    ...
    for node in is_player:
        ...
        for k in range(8):
            ...
            if (0 <= new_i < MAP_SIZE) and (0 <= new_j < MAP_SIZE) and map_state[new_i][new_j] == 0 and sheep_state[new_i][new_j] == 0:
                ...
                if cur_player_id==playerID and sheep_state[i][j] > 1:
                    sheeps=0
                ...
                elif cur_player_id!=playerID:
                    moves.append([(i, j), 2, dir_index[k]])
