...
def minimax(cur_player_id, map_state, sheep_state, depth, alpha, beta, max_player_id):
    ...
    if cur_player_id == max_player_id:
        max_score = -1000
...
def count_way(i, j):
    ...
    way = 0
    for t in range(8):
        if i + x[t] >= 0 and i + x[t] < 15 and j + y[t] >= 0 and j + y[t] < 15 and mapStat[i + x[t], j + y[t]] == 0:
            way += 1
...
def possible_move(cur_player_id, map_state, sheep_state):
    ...
    for node in is_player:
        ...
        for k in range(8):
            ...
            if (0 <= new_i < 15) and (0 <= new_j < 15) and map_state[new_i][new_j] == 0 and sheep_state[new_i][new_j] == 0:
                while (
                    (0 <= new_i + x[k] < 15)
                    and (0 <= new_j + y[k] < 15)
                ...
                if sheep_state[i][j] > 1:
                    sheeps=0
                    if cur_player_id==playerID:
                        if ori_way + new_way-1>0:
                            sheeps = sheep_state[i][j] * new_way / (ori_way + new_way-1)
                        if sheeps < 1 or ori_way + new_way-1<0:
                            sheeps = 1
                        if sheeps==sheep_state[i][j]:
                            sheeps=sheep_state[i][j]-1
                    else:
                        sheeps=sheep_state[i][j]/2
...
def choose_move(cur_player_id, map_state, sheep_state, move):
    ...
    di = dir_i[int((d - 1) / 3)]
    dj = dir_j[(d - 1) % 3]
    while 0 <= new_cor[0] + di < 15 and 0 <= new_cor[1] + dj < 15 and map_state[new_cor[0] + di][new_cor[1] + dj] == 0:
...
def evaluation(max_player_id, map_state, sheep_state):
    ...
    areas = []
    visited = [[0 for _ in range(15)] for _ in range(15)]
    for cor in cors:
...
def is_terminal(max_player_id, map_state, sheep_state):
    ...
    for cor in cors:
        ...
        for i in dir_i:
            for j in dir_j:
                if 0 <= cor[0] + i < 15 and 0 <= cor[1] + j < 15 and map_state[cor[0] + i][cor[1] + j] == 0:
                    return False
...
def InitPos(mapStat):
    ...
    boarder = []
    sheep_state = [[0 for _ in range(15)] for _ in range(15)]
    cur_id = 1
    for i in range(15):
        for j in range(15):
            if mapStat[i][j] == 0:
                for dir in dirs:
                    if 0 <= i + dir[0] < 15 and 0 <= j + dir[1] < 15 and mapStat[i + dir[0]][j + dir[1]] == -1 and [i, j] not in boarder:
                        boarder.append([i, j])
